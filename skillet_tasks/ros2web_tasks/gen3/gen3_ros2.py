...
@configclass
class Gen3Ros2WebEnvCfg(Ros2WebEnvCfg):
...
class Gen3Ros2WebEnv(Ros2WebEnv):
    ...
    def _decode_uint8_payload(payload: Any, field_name: str) -> np.ndarray:
        ...
        if isinstance(payload, dict):
            if "bytes" in payload:
                return Gen3Ros2WebEnv._decode_uint8_payload(payload["bytes"], field_name=field_name)
            if "data" in payload:
                return Gen3Ros2WebEnv._decode_uint8_payload(payload["data"], field_name=field_name)
