...
class HomeRelativeIKAction(DifferentialIKAction):
    ...
    def __init__(self, cfg: HomeRelativeIKActionCfg, env: ManagerBasedRlEnv) -> None:
        ...
        home_pos_local = (
            torch.tensor(cfg.home_pos, device=self.device, dtype=torch.float32)
            .unsqueeze(0)
        ...
        self._home_quat = (
            torch.tensor(cfg.home_quat, device=self.device, dtype=torch.float32)
            .unsqueeze(0)
            .expand(self.num_envs, -1)
            .clone()
        )
    ...
    def process_actions(self, actions: torch.Tensor) -> None:
        ...
        target_pos, target_quat = apply_delta_pose(self._home_pos, self._home_quat, delta)
        ...
        delta_clipped = torch.clamp(delta_from_prev, min=-self.cfg.max_pos_delta, max=self.cfg.max_pos_delta)
