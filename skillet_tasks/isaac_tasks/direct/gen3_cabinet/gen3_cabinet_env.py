...
class Gen3CabinetEnv(IsaacDirectRlEnv):
    ...
    cfg: Gen3CabinetEnvCfg

    def __init__(self, cfg: Gen3CabinetEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        lfinger_pose = get_env_local_pose(
            self.scene.env_origins[0],
            UsdGeom.Xformable(stage.GetPrimAtPath("/World/envs/env_0/Robot/left_inner_finger")),
            self.device,
        )
        rfinger_pose = get_env_local_pose(
            self.scene.env_origins[0],
            UsdGeom.Xformable(stage.GetPrimAtPath("/World/envs/env_0/Robot/right_inner_finger")),
            self.device,
        )
    ...
    def _get_observations(self) -> dict:
        dof_pos_scaled = (
            2.0
            * (self._robot.data.joint_pos[:, self.cfg.joint_ids] - self.robot_dof_lower_limits)
    ...
    def _compute_rewards(
        ...
        axis1 = tf_vector(gen3_grasp_rot, gripper_forward_axis)
        axis2 = tf_vector(drawer_grasp_rot, drawer_inward_axis)
        axis3 = tf_vector(gen3_grasp_rot, gripper_up_axis)
        ...
        lfinger_dist = gen3_lfinger_pos[:, 2] - drawer_grasp_pos[:, 2]
        rfinger_dist = drawer_grasp_pos[:, 2] - gen3_rfinger_pos[:, 2]
    ...
    def _compute_grasp_transforms(
        ...
        return global_gen3_rot, global_gen3_pos, global_drawer_rot, global_drawer_pos
