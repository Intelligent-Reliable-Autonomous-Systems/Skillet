...
def joint_posvel(base_cyclic) -> None:
    feedback = base_cyclic.RefreshFeedback()

    # Compute the current joint positions, velocities, and efforts
    return np.asarray([actuator.position for actuator in feedback.actuators]), np.asarray(
        [actuator.velocity for actuator in feedback.actuators]
    )


def deg_to_rad_wrapped(deg):
    # Shift angles > 180 to negative, then convert to radians
...
def example_send_joint_speeds(base, base_cyclic):
    ...
    if actuator_count == 6:
        ...
        for times in range(4):
            ...
            for _ in range(4):
                j, v = joint_posvel(base_cyclic)
                print(deg_to_rad_wrapped(j))
                print(np.deg2rad(v))
                print("\n")
                time.sleep(0.6)
...
def main():
    ...
    with utilities.DeviceConnection.createTcpConnection(args) as router:
        ...
        success &= example_send_joint_speeds(base, base_cyclic)
