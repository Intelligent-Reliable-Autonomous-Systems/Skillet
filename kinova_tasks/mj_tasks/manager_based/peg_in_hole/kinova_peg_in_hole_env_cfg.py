...
from kinova_tasks.assets.mujoco.kinova import KinovaClosedPegCfg
from kinova_tasks.assets.mujoco.peg_in_hole import get_hole_cfg, get_peg_cfg
...
def site_pos_relative_to_home(
    ...
    home_w = torch.tensor(home_pos, device=env.device).unsqueeze(0).expand(env.num_envs, -1) + env.scene.env_origins
...
def reset_gripper_joints_closed(
    env: ManagerBasedRlEnv,
    env_ids: torch.Tensor | None,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", joint_names=GRIPPER_JOINT_NAMES),
) -> None:
    ...
    pos = torch.tensor(list(GRIPPER_CLOSED_JOINT_POS.values()), device=env.device).unsqueeze(0).expand(n, -1).clone()
...
def reset_gripper_ctrl_closed(
    env: ManagerBasedRlEnv,
    env_ids: torch.Tensor | None,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", actuator_names=("fingers_actuator",)),
...
class ObservationsCfg:
    ...
    class CriticCfg(ObservationGroupCfg):
        ...
        # Peg start to hole top, peg end to hole bottom (6D)
        peg_to_hole = ObservationTermCfg(
            func=peg_to_hole_sites_distance,
            params={
                "peg_entity": "peg",
