...
class TestAsGymVectorEnv:
    """Tests for the AsGymVectorEnv adapter."""

    def _make_fake_env(
        self,
        num_envs: int = 4,
        obs_shape: tuple[int, ...] = (3,),
        act_shape: tuple[int, ...] = (2,),
    ) -> MagicMock:
        """Build a MagicMock pretending to be a gym.Env with vector-style attributes."""
        env = MagicMock(spec=gym.Env)
        single_obs = gym.spaces.Box(-1.0, 1.0, shape=obs_shape, dtype=np.float32)
        single_act = gym.spaces.Box(-1.0, 1.0, shape=act_shape, dtype=np.float32)
        batched_obs = gym.spaces.Box(-1.0, 1.0, shape=(num_envs, *obs_shape), dtype=np.float32)
        batched_act = gym.spaces.Box(-1.0, 1.0, shape=(num_envs, *act_shape), dtype=np.float32)

        attrs = {
            "num_envs": num_envs,
            "single_observation_space": single_obs,
            "single_action_space": single_act,
        }
        env.observation_space = batched_obs
        env.action_space = batched_act

        def _get_wrapper_attr(name: str) -> Any:
            if name in attrs:
                return attrs[name]
            raise AttributeError(name)

        def _has_wrapper_attr(name: str) -> bool:
            return name in attrs

        env.get_wrapper_attr = _get_wrapper_attr
        env.has_wrapper_attr = _has_wrapper_attr
        return env

    def test_basic_construction(self):
        fake = self._make_fake_env(num_envs=5)
        wrapper = AsGymVectorEnv(fake, num_envs=5)
        assert wrapper.num_envs == 5
        assert wrapper.single_observation_space == fake.get_wrapper_attr("single_observation_space")
        assert wrapper.single_action_space == fake.get_wrapper_attr("single_action_space")

    def test_num_envs_inferred_from_env(self):
        fake = self._make_fake_env(num_envs=7)
        wrapper = AsGymVectorEnv(fake)
        assert wrapper.num_envs == 7

    def test_raises_without_num_envs(self):
        env = MagicMock(spec=gym.Env)
        env.get_wrapper_attr = MagicMock(side_effect=AttributeError)
        env.has_wrapper_attr = MagicMock(return_value=False)
        with pytest.raises((ValueError, AttributeError)):
            AsGymVectorEnv(env)

    def test_raises_without_single_spaces(self):
        env = MagicMock(spec=gym.Env)
        env.get_wrapper_attr = MagicMock(return_value=4)
        env.has_wrapper_attr = MagicMock(return_value=False)
        with pytest.raises(ValueError, match="single observation space"):
            AsGymVectorEnv(env, num_envs=4)

    def test_reset_delegates(self):
        fake = self._make_fake_env()
        expected_obs = np.zeros((4, 3))
        fake.reset.return_value = (expected_obs, {"k": "v"})
        wrapper = AsGymVectorEnv(fake)

        obs, info = wrapper.reset(seed=42, options={"x": 1})
        fake.reset.assert_called_once_with(seed=42, options={"x": 1})
    ...
    def test_step_delegates(self):
        fake = self._make_fake_env()
        actions = np.ones((4, 2))
        step_return = (np.zeros((4, 3)), np.array([1.0] * 4), np.array([False] * 4), np.array([False] * 4), {})
        fake.step.return_value = step_return
        wrapper = AsGymVectorEnv(fake)

        result = wrapper.step(actions)
        fake.step.assert_called_once_with(actions)
        assert len(result) == 5

    def test_render_delegates(self):
        fake = self._make_fake_env()
        fake.render.return_value = "frame"
        wrapper = AsGymVectorEnv(fake)
        assert wrapper.render() == "frame"
    ...
    def test_is_vector_env_subclass(self):
        fake = self._make_fake_env()
        wrapper = AsGymVectorEnv(fake)
        assert isinstance(wrapper, gym.vector.VectorEnv)

    def test_spaces_match_inner_env(self):
        fake = self._make_fake_env(num_envs=2, obs_shape=(5,), act_shape=(3,))
        wrapper = AsGymVectorEnv(fake)
        assert wrapper.observation_space == fake.observation_space
        assert wrapper.action_space == fake.action_space
