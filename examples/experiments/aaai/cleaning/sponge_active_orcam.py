...
parser.add_argument(
    "--exp_config",
    type=str,
    default="skillet_tasks/spongeworld-clean/repair-sponge/repair-effects-orcam.json",
...
def main() -> None:
    ...
    sponge_domain = "skillet_tasks/spongeworld-clean/simple-sponge.domain.pddl"
    env_cfg = {
    ...
    abs_model = AbstractModel(
        sponge_domain, None, scene, environment=learning_agent.orcam.converter.domain.environment, domain="sponge"
    )
    ...
    place_skill = PlaceSkill(
        reach_policy=arm_policy, lift_height=0.21, gripper_close=0.67, length=skill_length, default_quat=[[0, 1, 0, 0]]
    )
