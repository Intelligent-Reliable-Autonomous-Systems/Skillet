...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    if args_cli.robot == "panda":
        ...
        ee_frame_name = "end_effector_link"  # NO
        arm_joint_names = ["joint_.*"]
        ee_frame_idx = robot.find_bodies(ee_frame_name)[0][0]
        arm_joint_ids = robot.find_joints(arm_joint_names)[0]
    ...
    joint_centers = torch.nan_to_num(
        torch.mean(robot.data.soft_joint_pos_limits[:, arm_joint_ids, :], dim=-1), nan=0.0, posinf=0.0, neginf=0.0
    )
    ...
    while simulation_app.is_running():
        ...
        if count % 500 == 0:
            ...
            mass_matrix = robot.root_physx_view.get_generalized_mass_matrices()[:, arm_joint_ids, :][
                :, :, arm_joint_ids
            ]
