...
Test file for executor integration Skillet skills in Isaac
...
if TYPE_CHECKING:
    ...
    from skillet.envs.specs import BxM_Action, OSC_Obs
from skillet.agents.policy_over_options import PolicyOverOptionsAgent, PolicyOverOptionsBatchedAgent
...
def main() -> None:
    ...
    # Low-level policies
    ik_ee_pose_policy = PoseAbsIKEEPolicy(env.obs_spec_osc, env.action_spec)

    # Skills
    skill_length = 100
    reach_pose_skill = ReachPoseSkill(name="reach_pose_skill", policy=ik_ee_pose_policy, length=skill_length)
    skills: list[BatchedSkill[OSC_Obs, BxM_Action, XYZ_QUAT_Params]] = [reach_pose_skill]
