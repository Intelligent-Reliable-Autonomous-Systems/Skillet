...
def _is_in(a: Sponge | Location, b: Bin, height_tol_frac: float = 0.3, xy_slack_frac: float = 0.5) -> bool:
    """Return True if sponge *a* is inside Bin *b*.
    ...
    """
    if isinstance(a, Sponge) and isinstance(b, Bin) and not (a.is_pose_known() and b.is_pose_known()):
        return False
    ...
    a_cx = (aabb_a[0] + aabb_a[3]) / 2.0
    a_cy = (aabb_a[1] + aabb_a[4]) / 2.0
    a_cz = (aabb_a[2] + aabb_a[5]) / 2.0

    within_x = (aabb_b[0] - slack) <= a_cx <= (aabb_b[3] + slack)
    within_y = (aabb_b[1] - slack) <= a_cy <= (aabb_b[4] + slack)

    # a's z center should fall within b's footprint (plus a little slack)
    within_z = (aabb_b[2] - tol) <= a_cz <= (aabb_b[5] + tol)

    return bool(within_x and within_y and within_z)


def _is_hovering(a: Cube | Location, b: Cube | Location, xy_slack_frac: float = 0.5) -> bool:
    """Return True if object a is hovering object b.
...
def ground_sponge_relations(scene: Scene) -> tuple[list[tuple[str, SceneObject, SceneObject]], ...]:
    ...
    for obj in scene.objects:
        ...
        if isinstance(obj, Can) and obj.pose[2] > 0.07:
            material_relations.append(("upright", obj))
