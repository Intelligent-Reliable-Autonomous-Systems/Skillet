...
class NativeMujocoViewer(BaseViewer):
    ...
    def setup(self) -> None:
        ...
        # self._term_names = [
        #     name for name, _ in self.env.unwrapped.reward_manager.get_active_iterable_terms(self.env_idx)
        # ]
    ...
    def _update_reward_figures(self, viewer: mujoco.viewer.Handle) -> None:
        ...
        # terms = list(self.env.unwrapped.reward_manager.get_active_iterable_terms(self.env_idx))
        ...
        viewports = compute_viewports(len(self._term_names), viewer.viewport, self._plot_cfg)
        viewport_figs = [
            (viewports[i], self._figures[self._term_names[i]])
            for i in range(min(len(viewports), len(self._term_names), self._plot_cfg.max_viewports))
        ]
    ...
    def _update_debug_visualizers(self, viewer: mujoco.viewer.Handle) -> None:
        ...
        if self._show_debug_vis and hasattr(self.env.unwrapped, "update_visualizers"):
            assert self.mjm is not None
            visualizer = MujocoNativeDebugVisualizer(viewer.user_scn, self.mjm, self.env_idx, self._show_all_envs)
            self.env.unwrapped.update_visualizers(visualizer)

    def _sync_env_state_to_mjdata(self, target_data: mujoco.MjData, sim_data: _SimDataProtocol, env_idx: int) -> None:
    ...
    def _render_other_env_geoms(
        ...
        for i in range(self.env.unwrapped.num_envs):
            ...
            mujoco.mjv_addGeoms(self.mjm, self.vd, self.vopt, self.pert, self.catmask, viewer.user_scn)
    ...
    def _stabilize_tracking_camera(self) -> None:
        ...
        """
        assert self.mjd is not None
        if not (self.viewer and self.viewer.cam.type == mujoco.mjtCamera.mjCAMERA_TRACKING.value):
            return
    ...
    def sync_viewer_to_env(self) -> None:
        ...
        if pert.active != 0 and pert.select > 0:
            ...
            sim_data.qfrc_applied[self.env_idx] = torch.from_numpy(qfrc).to(device=sim_data.qfrc_applied.device)
    ...
    def _setup_camera(self) -> None:
        ...
        self.viewer.cam.elevation = getattr(self.cfg, "elevation", self.viewer.cam.elevation)
    ...
    def _set_camera_asset_body(self) -> None:
        ...
        if self.cfg.body_name not in robot.body_names:
            raise ValueError(f"Body '{self.cfg.body_name}' not found in asset '{self.cfg.entity_name}'")
