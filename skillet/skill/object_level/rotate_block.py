...
class RotateBlockSkill(SingleSkill[IKEE_Obs, M_Action, Object_Params]):
    """A skill for rotating a block in the scene.

    Is is discretely parameterized by the id of the block to rotate.
    """

    def __init__(
        self,
        scene: Scene,
        rotate_skill: RotateYawSkill[BxM_Action],
        vis_target_pos: Callable[[Sequence[float]], None] | None = None,
    ) -> None:
        """Initialize the rotate block skill."""
        self._scene = scene
        self._rotate_skill = rotate_skill
    ...
    def initiate(self, obs, params):
        ...
        self._rotate_skill.initiate(obs, target_pose)

    @override
    def get_action(self, obs: IKEE_Obs) -> M_Action:
        obs = self._rotate_skill.obs_spec.cast(obs)
        actions = self._rotate_skill.get_action(obs)
    ...
    def status(self) -> SkillStatus:
        ...
        if self._status is not None:
            return self._status
        return self._rotate_skill.status[0]
