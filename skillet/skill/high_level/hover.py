"""A hover skill for moving the end effector over a surface."""
...
class HoverStatusCodes(IntEnum):
...
class HoverSkill(BatchedSkill[IKEE_Obs, TBAction, XYZ_YAW_Params], Generic[TBAction]):
    """A hover skill for hovering an object up at a location and lifting to a desired height.

    Generic Args:
        TBAction: The type of the action for the skill.

    Parameterized by [x,y,z, yaw] the x y z location to perform the hover action and orientation
    """
    ...
    def initiate(self, obs: TBSkillObs, params: TBSkillParams) -> None:
        """Initiate the hover skill.

        Args:
            obs: The low-level observation for the skill.
            params: The hover parameters, (x, y, z, yaw) as shape (b, 4)
        ...
        self._hover_status = spec.zeros(shape=(self.n_envs,), dtype=int)
        self._status[:] = SkillStatusCodes.RUNNING
        self._hover_status[:] = HoverStatusCodes.ASCEND
        self._params = params
        self._n_steps = 0
        ...
        # Define the target poses for each stage of the hover skill, indexed by HoverStatusCodes
        # (n_envs, num_hover_stages, 7)
        target_poses = spec.zeros(shape=(self.n_envs, 7, 7), dtype=float)
        # ASCEND[1]: Go up to lift height (gripper open)
        target_poses[:, HoverStatusCodes.ASCEND, :7] = ee_pose_b
        target_poses[:, HoverStatusCodes.ASCEND, 2] = self._lift_height

        # HOVER[2]: Go over to the target x,y position, oriented downward (gripper open)
        target_poses[:, HoverStatusCodes.HOVER, :2] = params[:, :2]  # (x,y) from params
        target_poses[:, HoverStatusCodes.HOVER, 2] = self._lift_height
        target_poses[:, HoverStatusCodes.HOVER, 3:7] = goal_quat
        # LOWER[3]: Go down to the target z position (gripper open)
        target_poses[:, HoverStatusCodes.LOWER, :7] = target_poses[:, HoverStatusCodes.HOVER, :7]
        target_poses[:, HoverStatusCodes.LOWER, 2] = params[:, 2]
        # GRASP[4]: Close gripper
        target_poses[:, HoverStatusCodes.GRASP, :7] = target_poses[:, HoverStatusCodes.LOWER, :7]
        # LIFT[5]: Lift up to the target z position (gripper closed)
        target_poses[:, HoverStatusCodes.LIFT, :7] = target_poses[:, HoverStatusCodes.HOVER, :7]
        ...
        self._current_target_poses = target_poses[idx, self._hover_status]
    ...
    def get_action(self, obs: TBSkillObs) -> TBAction:  # noqa: D102
        ...
        reached_height = self._hover_status == HoverStatusCodes.ASCEND & (
        ...
        if next_pose.any():
            ...
            self._hover_status[valid_idx] += 1
            valid_idx = valid_idx & (self._hover_status < HoverStatusCodes.DONE)
            # print(
            #     f"[INFO][HOVER STATUS UPDATE]: {HoverStatusCodes(self._hover_status.cpu().numpy()[0]).name} | reached_pose: {reached_pose.cpu().numpy()}"
            # )
            # Update the target pose based on the new hover status
            self._current_target_poses[valid_idx] = self._target_poses[idx[valid_idx], self._hover_status[valid_idx]]
        ...
        reach_actions[:, -1] = torch.ones_like(reach_actions[:, -1]) * self._gripper_close  # Close gripper

        stuck = (self._hover_status != HoverStatusCodes.GRASP) & self._target_manager.is_stuck()
        if stuck.any():
            self._status[stuck] = SkillStatusCodes.FAILED
        self._n_steps += 1
        self._status[self._hover_status == HoverStatusCodes.DONE] = SkillStatusCodes.SUCCESS
