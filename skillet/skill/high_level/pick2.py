...
class PickSkill(BatchedSkill[TBSkillObs, TBAction, TBSkillParams], Generic[TBSkillObs, TBAction, TBSkillParams]):
    ...
    def get_action(self, obs: TBSkillObs) -> TBAction:  # noqa: D102
        ...
        print(f"[INFO][PICK STATUS]: {self._pick_status.cpu().numpy()[0]} | target pose: {self._current_target_poses.cpu().numpy()[0]} | obs tcp pose: {obs['tcp_pose_b'].cpu().numpy()[0]}")

        # prev_pick_status = self._pick_status.clone()

        ee_pose_b = obs["tcp_pose_b"]
        reached_pos = torch.linalg.vector_norm(ee_pose_b[:, 0:3] - self._current_target_poses[:, 0:3],
            dim=1) < self._pos_threshold
        reached_height = self._pick_status == PickStatusCodes.ASCEND \
            & (ee_pose_b[:, 2] >= self._current_target_poses[:, 2])
        reached_quat = quat_error_magnitude(ee_pose_b[:, 3:7], self._current_target_poses[:, 3:7]) < self._quat_threshold
