...
class OnPolicyRunner:
    ...
    def _construct_algorithm(self, obs: TensorDict) -> PPO:
        ...
        num_skills = len(self.env.cfg.skills) if hasattr(self.env.cfg, "skills") else None
        # Initialize the policy
        actor_critic_class = eval(self.policy_cfg.pop("class_name"))
        actor_critic: ActorCritic | ActorCriticRecurrent = actor_critic_class(
            obs, self.cfg["obs_groups"], self.env.num_actions, num_skills=num_skills, **self.policy_cfg
        ).to(self.device)
    ...
    def _prepare_logging_writer(self) -> None:
        ...
        if self.log_dir is not None and self.writer is None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                from skillet.rl.rsl_rl.utils.neptune_utils import NeptuneSummaryWriter

                self.writer = NeptuneSummaryWriter(log_dir=self.log_dir, flush_secs=10, cfg=self.cfg)
                self.writer.log_config(self.env.cfg, self.cfg, self.alg_cfg, self.policy_cfg)
            elif self.logger_type == "wandb":
                from skillet.rl.rsl_rl.utils.wandb_utils import WandbSummaryWriter
