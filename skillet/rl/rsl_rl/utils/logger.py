...
class Logger:
    ...
    def init_logging_writer(self) -> None:
        """Initialize the logging writer, which can be Tensorboard."""
        if self.log_dir is not None and not self.disable_logs:
            ...
            if self.logger_type == "tensorboard":
                ...
                raise ValueError("Logger type not found. Please choose 'tensorboard'.")
        else:
            self.writer = None
    ...
    def _store_code_state(self) -> list[str]:
        ...
        if self.log_dir is not None and not self.disable_logs:
            ...
            pathlib.Path(git_log_dir).mkdir(exist_ok=True, parents=True)
            ...
            for repository_file_path in self.git_status_repos:
                ...
                if pathlib.Path(diff_file_name).is_file():
                    continue
                # Write the diff file
                print(f"Storing git diff for '{repo_name}' in: {diff_file_name}")
                with pathlib.Path(diff_file_name).open("x", encoding="utf-8") as f:
